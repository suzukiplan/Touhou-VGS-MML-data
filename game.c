	...
	for(i=0;i<SONG_NUM;i++) {
		...
		if(i<bExist && 114<dp && dp<320) {
			...
			if(_list[i].dis) {
				...
				putkanji(26,i*20+133+(_list==_listE?1:0)+(int)base,103,"%s",_list[i].text);
			} else {
				putfontS(8,i*20+137+(int)base,"%3d.",(i+1));
				putkanji(27,i*20+134+(_list==_listE?1:0)+(int)base,1,"%s",_list[i].text);
				putkanji(26,i*20+133+(_list==_listE?1:0)+(int)base,255,"%s",_list[i].text);
			}
		}
	}
